...
def run_check(domains: list[str], max_depth: int = 2, use_spa: bool = False) -> None:
    ...
    for raw_domain in domains:
        ...
        print(f"\n[1/4] Generating seed URLs for {domain}...")
        seeds = generate_seeds(domain)
        for s in seeds:
            print(f"       {s}")

        print(f"\n[2/4] Starting BFS crawl (max_depth={max_depth})...")
        pages = bfs_crawl(seeds, domain, max_depth=max_depth, use_spa=use_spa)
        print(f"       Crawled {len(pages)} pages")

        print(f"\n[3/4] Classifying content chunks...")
        ...
        for url, text in pages:
            ...
            for chunk in chunks:
                ...
                if result["compliant"]:
                    print(f"         ✓ Found: {', '.join(result['standards'])}")
...
def main() -> None:
    ...
    check_parser.add_argument("--depth", type=int, default=2, help="BFS crawl depth")
